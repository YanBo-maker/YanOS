...
| CTest 汇总 | M1a 阶段按当时的 `CMakeLists.txt` 实测的**注册数**（M2a 之后套件数已变，当前值见 [STATUS](../STATUS.md)）：只做本地模块测试时 21 个；加 `-DYAN_BUILD_TOOLS=ON` 为 22 个；再给出 `YAN_NEMU_REF_SO`、`YAN_RISCV_TESTS_DIR`、`YAN_RISCV_ARCH_TEST_DIR`、`YAN_SAIL_BIN` 为 28 个；再加上 `YAN_NEMU_REF_DIR` 共 29 个。这是注册数而不是通过数：各组的通过情况以 [STATUS](../STATUS.md) 的验证一节为准。外部验证层的测试只在 `YAN_BUILD_TOOLS=ON` 时注册，因此上一阶段记录的 25 / 27 不能按 +2 机械推算 |
...
模块测试开启 AddressSanitizer 与 UndefinedBehaviorSanitizer 后同样通过；`uart`、`transport`、`plic` 三组的当前结论与变异检查见 [STATUS](../STATUS.md) 的验证一节。
