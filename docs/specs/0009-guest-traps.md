...
把 CPU 的指令与访存故障交给 Guest 处理，完成异常进入、CSR 访问和异常返回。当前仅支持 M-mode，采用直接异常入口，不实现中断、U/S 模式、分页或委托。
...
| 0x301 / 0x304 / 0x310 / 0x344 | misa / mie / mstatush / mip | 固定读零，写入忽略；misa 暂不提供 ISA 探测值 |
