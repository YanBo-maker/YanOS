...
当前阶段：CPU 条件分支与跳转。实现与本地验证已完成，用户审查待完成。整数计算阶段已通过用户审查并合入主分支。
...
GCC 11.4、CMake 3.22.1 下的 Debug 与 Release 构建均通过八组 CTest：RAM、Bus、Machine、CPU、Fetch、Step、ALU、Control，共 51 个 Unity 测试用例。
...
条件分支、JAL、JALR 的新增执行测试均确认在对应实现前失败，实现后通过。
...
审查 [控制转移规格](specs/0007-control-flow.md) 及配套实现，随后定义 Load / Store 指令和数据访问错误行为。

当前支持 29 条 RV32I 整数计算与控制转移指令；其余编码返回 Host 状态 `YAN_UNSUPPORTED_INSTRUCTION`。目标未对齐返回 `YAN_UNALIGNED`，Guest 异常机制仍待实现。访存、M 扩展、FENCE、CSR、中断和设备尚未实现。Machine 装载调用者提供的内存缓冲区，文件读取及 Guest 启动仍待实现。
