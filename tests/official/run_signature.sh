...
#
# Exit codes: 0 all cases matched, 1 at least one differed or a tool this check
# drives is missing from the repository, 2 usage, 77 this machine lacks a
# dependency that lives outside the repository (the toolchain, Sail, python3 or
# its nm); CTest reports 77 as SKIP, never as PASS.
set -u
...
# toolchain, the Sail reference model, python3 and the toolchain's nm all live
# outside the repository, so a missing one stays a skip. yan_run and yan_gen are
# built from this repository: a missing one is a build that did not happen, so it
# fails instead. Deleting either tool must never be a way to a green suite - CTest
